...
local function determine_code(text_colour, background_colour, variable)
	...
	local undefined_ct_code = ("c-%s__x-%s"):format(text_colour, background_colour)
	table.insert(MuExporter.undefined_ct_codes, undefined_ct_code)
	return undefined_ct_code
end
		...
		for _,line in ipairs(box) do
			...
			for i=1, (#line_prepare - 2) do
				...
				if (
					...
					and next_seg.ct_code -- Check required as there is no nil ct_code
					and cross_seg.ct_code and not cross_seg.s
					and cross_seg.ct_code == segment.ct_code
				) then
					line_prepare[i].final_text = line_prepare[i].final_text:gsub("}}", "")
					line_prepare[i+2].final_text = line_prepare[i+2].final_text:gsub(("{{ct|%s|"):format(line_prepare[i+2].ct_code), "")
				end

				-- Merging size groups
				if (
					segment.s and next_seg.s and segment.s == next_seg.s
				) then
					line_prepare[i].final_text = line_prepare[i].final_text:gsub("</big>",""):gsub("</small>","")
					line_prepare[i+1].final_text = line_prepare[i+1].final_text:gsub("<[^/]big>",""):gsub("<[^/]small>","")
				end

				table.insert(line_concat, line_prepare[i].final_text)
			end
			table.insert(line_concat, line_prepare[#line_prepare-1].final_text)
			table.insert(line_concat, line_prepare[#line_prepare].final_text)
			table.insert(parsed_table, table.concat(line_concat))
		end
